...
class State {
  ...
  createInitialState() {
    ...
    this.initialState = { ...this.initialState, ...this.addDataFieldToCustomInitialState(this.customInitialState) }
    this.initialState = fromJS(this.initialState)
  }

  addDataFieldToCustomInitialState = customInitialState => {
    const newCustomInitialState = {}
    for (const key in customInitialState) {
  ...
  extractDataForDispatch(data) {
    ...
    payload = Object.keys(payload).reduce((acc, key) => {
      if (this.schema[key.type] === 'arrayOfObjects' && isPlainObject(payload[key])) {
    ...
    const updateParentReference = (data, key, parentKey) => {
      const parent = Array.isArray(data[parentKey]) ? data[parentKey] : [data[parentKey]]
  ...
  merge(merger, options = {}) {
    if (!merger) return
    const { type = 'stateMerge', key = 'merge' } = options
    const newState = merger(this.store.getState())
    if (newState) {
      this.store.dispatch({
        type: `${type}/merge/success`,
        autoReducer: true,
        payload: fromJS({
  ...
  query = async ({
    ...
    try {
      ...
      if (changeExtractedData) {
        ...
        extractedData = changeExtractedData(
          extractedData,
          data,
          this.store.getState(),
          type.split('/')[0]
